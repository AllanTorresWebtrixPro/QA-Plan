...
export function SurveyTestSummary({
  tests,
  currentUser,
}: SurveyTestSummaryProps) {
  ...
  const highPrioritySurveyTests = surveyTests.filter(
    (test) => test.priority === "High"
  );
  const completedHighPriority = highPrioritySurveyTests.filter(
    (test) => test.completed
  );

  const surveyProgress =
    surveyTests.length > 0
      ? (completedSurveyTests.length / surveyTests.length) * 100
      : 0;
  ...
  const surveyCategories = [
    {
      ...
      tests: surveyTests.filter(
        (t) =>
          t.id.startsWith("srv-00") && Number.parseInt(t.id.split("-")[1]) <= 3
      ),
      icon: BarChart3,
    },
    {
    ...
    },
    {
      ...
      tests: surveyTests.filter(
        (t) =>
          t.id.startsWith("srv-0") && Number.parseInt(t.id.split("-")[1]) >= 30
      ),
      icon: AlertTriangle,
    },
  ];
        ...
        <CardContent>
          ...
          <div className="mt-4 flex items-center justify-between text-sm">
            ...
            <span>
              ...
              {highPrioritySurveyTests.length}
            </span>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {["High", "Medium", "Low"].map((priority) => {
              const priorityTests = surveyTests.filter(
