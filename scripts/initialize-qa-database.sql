...
-- This script safely creates all tables and data, handling existing objects
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'qa_user_test_progress_user_id_fkey'
    ) THEN
        ALTER TABLE qa_user_test_progress 
        ADD CONSTRAINT qa_user_test_progress_user_id_fkey 
        FOREIGN KEY (user_id) REFERENCES qa_users(id) ON DELETE CASCADE;
    END IF;

    -- Add foreign key for test_id if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'qa_user_test_progress_test_id_fkey'
    ) THEN
        ALTER TABLE qa_user_test_progress 
        ADD CONSTRAINT qa_user_test_progress_test_id_fkey 
        FOREIGN KEY (test_id) REFERENCES qa_tests(id) ON DELETE CASCADE;
    END IF;
END $$;
...
-- Insert default test cases (only if they don't exist)
...
SELECT 'Users created: ' || count(*) as user_count FROM qa_users;
SELECT 'Tests created: ' || count(*) as test_count FROM qa_tests;
